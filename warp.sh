...
readp "Set the device name, press Enter to randomize:" dname
if [[ -z $dname ]]; then
...
green "WARP+ account upgraded successfully!" && ShowWGCF && WGCFmenu && break
else
...
yellow "1. Check whether the WARP+ account in the 1.1.1.1 APP or the secret key shared by the network has traffic"
...
reswarp
checkwgcf
[[ $wgcfv4 =~ on|plus || $wgcfv6 =~ on|plus ]] && green "Successful opening of warp" || red "Failed to open warp"
else
WARPonoff
fi
}
...
WARPIPv6Status=$(white "WARP status:\c" ; green "Running, WARP ordinary account (unlimited WARP traffic)" ; white "Service provider Cloudflare obtains IPV6 address:\c" ; green "$v6 $country" ; white " Netflix NF unlocking status: \c" ; green "$NF" ; white " ChatGPT unlocking status: \c" ; green "$chat");;
off)
WARPIPv6Status=$(white "WARP status: \c" ; yellow "Close" ; white " Service provider $isp6 obtains IPV6 address: \c" ; yellow "$v6 $country" ; white " Netflix NF unlocking status: \ c" ; yellow "$NF" ; white " ChatGPT unlocking status:\c" ; yellow "$chat");;
esac
else
WARPIPv6Status=$(white "IPV6 status: \c" ; red "IPV6 address does not exist ")
fi
...
checkwgcf
[[ $wgcfv4 =~ on|plus || $wgcfv6 =~ on|plus ]] && green "Congratulations! The IP of warp was obtained successfully!" && break || red "Sorry! The IP of warp failed to be obtained"
done
checkwgcf
if [[ ! $wgcfv4 =~ on|plus && ! $wgcfv6 =~ on|plus ]]; then
...
do
yellow "When applying for a warp ordinary account, you may be prompted multiple times: 429 Too Many Requests, please wait 30 seconds" && sleep 1
...
yellow "warp+ cannot be automatically generated, directly generate a warp ordinary account"
fi
...
WARPup(){
backconf(){
red "Upgrade failed, automatically restore normal warp account"
sed -i "2s#.*#$(sed -ne 2p /etc/wireguard/wgcf-profile.conf)#;4s#.*#$(sed -ne 4p /etc/wireguard/wgcf-profile.conf) #" /etc/wireguard/wgcf.conf
CheckWARP && ShowWGCF && WGCFmenu
}
...
[[ ! $(type -P wg-quick) ]] && red "wgcf-warp is not installed, and wgcf-warp must be installed before execution" && exit
...
2 )
[[ ! $(type -P wg-quick) ]] && red "wgcf-warp is not installed, and wgcf-warp must be installed before execution" && exit
readp "Please copy the key license key in the WARP+ state of the mobile WARP client or the key shared by the network (26 characters):" ID
[[ -n $ID ]] && sed -i "s/license_key.*/license_key = '$ID'/g" /etc/wireguard/wgcf-account.toml && readp "Rename device name (just press Enter to name randomly):" sbmc || (red "No key license key (26 characters) was entered" && cf)
...
flow=`echo "scale=2; $warppflow/1000000000" | bc`
green "Upgraded to wgcf-warp+ account\nwgcf-warp+ account device name: $(grep -s 'Device name' /etc/wireguard/wgcf+p.log | awk '{ print $NF }')\nwgcf-warp+ Account remaining flow: $flow GB"
ShowWGCF && WGCFmenu
else
red "After IP detection, the upgrade to warp+ failed. Please ensure that the key is used on no more than 5 devices. It is recommended to change the key and try again. The script exits" && exit
fi;;
3)
checkwgcf
if [[ $wgcfv4 = plus || $wgcfv6 = plus ]]; then
...
yellow "Currently a normal wgcf-warp account"
ShowWGCF && WGCFmenu
fi;;
4)
SOCKS5WARPUP;;
...
yellow "Installing necessary dependencies for the first time...please wait..."
$yumapt update ; $yumapt install curl wget bc qrencode python3 screen cron ; touch warp_update
